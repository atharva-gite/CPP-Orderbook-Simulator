...
OrderBook::OrderBook(size_t expected_orders, AllocMode alloc)
    : id_map_(), next_order_id_(1), pool_(expected_orders, alloc)
{
    id_map_.reserve(expected_orders * 2);
}

OrderBook::~OrderBook() {
    // Heap-mode Orders are individual allocations; Pool-mode returns slots
    // to the slab which ~OrderPool then releases in one shot.
    for (auto& kv : id_map_) {
        pool_.deallocate(kv.second);
    }
    id_map_.clear();
...
Order* OrderBook::detach_pending(order_id_t id) noexcept {
    ...
    // remove from id_map_
    auto mit = id_map_.find(id);
    if (mit != id_map_.end()) {
        if (!found) found = mit->second;
        id_map_.erase(mit);
    }
    return found;
}
...
bool OrderBook::configure_iceberg(order_id_t id, qty_t display) noexcept {
    auto it = id_map_.find(id);
    if (it == id_map_.end() || display == 0) return false;
    Order* o = it->second;
    ...
    if (static_cast<Side>(o->side) == Side::Buy) {
        ...
        if (lit != bids_.end()) lit->second.adjust_aggregate(delta);
    } else {
        auto lit = asks_.find(o->price);
        if (lit != asks_.end()) lit->second.adjust_aggregate(delta);
    }
    return true;
}
...
qty_t OrderBook::apply_fill(order_id_t id, qty_t qty) noexcept {
    ...
    if (static_cast<Side>(o->side) == Side::Buy) {
        auto lit = bids_.find(o->price);
        if (lit != bids_.end()) {
            lit->second.adjust_aggregate(-static_cast<std::int64_t>(filled));
            lvl = &lit->second;
        }
    } else {
        auto lit = asks_.find(o->price);
    ...
    if (o->remaining_qty == 0 && lvl) {
        ...
        if (lvl->order_count() == 0) {
            if (static_cast<Side>(o->side) == Side::Buy) bids_.erase(o->price);
...
order_id_t OrderBook::add_limit_order(Side side, price_t price, qty_t qty) {
    ...
    if (side == Side::Buy) {
        auto it = bids_.find(price);
        if (it == bids_.end()) it = bids_.emplace(price, PriceLevel{}).first;
        it->second.push_back(o);
    } else {
        auto it = asks_.find(price);
        if (it == asks_.end()) it = asks_.emplace(price, PriceLevel{}).first;
        it->second.push_back(o);
    }
...
bool OrderBook::cancel_order(order_id_t id) noexcept {
    ...
    if (static_cast<Side>(o->side) == Side::Buy) {
        ...
        if (lit != bids_.end()) {
            lit->second.remove(o);
            if (lit->second.order_count() == 0) bids_.erase(lit);
        }
    } else {
        auto lit = asks_.find(o->price);
        if (lit != asks_.end()) {
            lit->second.remove(o);
            if (lit->second.order_count() == 0) asks_.erase(lit);
        }
    }
    id_map_.erase(it);
    pool_.deallocate(o);
    ...
    if (new_price == old_price) {
        ...
        if (static_cast<Side>(o->side) == Side::Buy) {
            ...
            if (lit != bids_.end()) {
                ...
                lit->second.adjust_aggregate(delta_rem);
            }
        } else {
...
qty_t OrderBook::best_bid_qty() const noexcept {
    ...
    return bids_.begin()->second.aggregate_qty();
}

qty_t OrderBook::best_ask_qty() const noexcept {
    if (asks_.empty()) return 0;
    return asks_.begin()->second.aggregate_qty();
}
...
std::string OrderBook::snapshot(size_t max_levels) const {
    ...
    for (auto it = asks_.begin(); it != asks_.end() && cnt < max_levels; ++it, ++cnt) {
        ss << it->first << " qty=" << it->second.aggregate_qty() << " orders=" << it->second.order_count() << "\n";
    }
    ss << "BIDS:\n";
    cnt = 0;
